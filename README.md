...
项目适合初学者按顺序阅读和跟着跑实验, 每个章节都有详细的理论理解以及对应的代码讲解和实验.

## 你会在这里看到什么

### 我个人的研究理解

这个仓库不是对论文, 教程或开源代码的简单照搬, 而是一份带有研究性质的 Transformer 学习笔记.

我会尽量尊重经典论文和主流实现, 但也会加入自己的理解, 推导和实验判断. 比如在讲 Attention 时, 我不会只停留在Q 是查询, K 是键, V 是值的比喻上, 而是更关注 `QK^T` 如何构造 token 之间的关系矩阵, 在讲多头注意力, RoPE 和 Transformer Block 时, 也会围绕这个模块到底给模型带来了什么能力来展开.

因此, 你会看到一些带有个人思考的解释, 实验取舍和实现方式.

### 从零拆解 Transformer 的核心组件

这个仓库把 Transformer 拆成多个小模块, 按照学习顺序逐步复现. 你会先看到单头注意力, 再到多头注意力, RoPE 位置编码, Transformer Block, 最后组装成可训练的 Transformer. 每一章都尽量围绕一个明确的概念展开, 方便从公式是什么过渡到代码怎么写.

### 可以直接运行和观察的 Notebook

内容按章节推进, 从单头注意力开始, 逐步加入多头机制, RoPE, Transformer Block, 最后到一个可训练的 Encoder/Decoder. 我的写法会尽量保持朴素, 不依赖复杂数学推理, 全部代码实验均可复现, 方便直接对照张量形状和计算过程.
...
首先就是经典论文*Attention Is All You Need*, 没有这篇论文也就没有现在的大语言模型. <https://arxiv.org/abs/1706.03762>
